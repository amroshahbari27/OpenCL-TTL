...
__TTL_TRACE_FN(TTL_step_buffering, TTL_IMPORT_DOUBLE_BUFFERING_TYPE *const db, const TTL_tile_t next_tile) {
    ...
    const TTL_layout_t int_layout =
        TTL_create_layout(next_tile.shape.width, next_tile.shape.width * next_tile.shape.height);
    ...
    const TTL_layout_t prev_int_layout = TTL_create_layout(
        db->prev_tile.shape.width, db->prev_tile.shape.width * db->prev_tile.shape.height);
...
__TTL_TRACE_FN(TTL_step_buffering, TTL_EXPORT_DOUBLE_BUFFERING_TYPE *const db, TTL_tile_t tile_current) {
    const TTL_layout_t int_layout = TTL_create_layout(
        db->prev_tile.shape.width, db->prev_tile.shape.width * db->prev_tile.shape.height);
    ...
    db->common.index = (db->common.index + 1) % 2;  // TTL_ARRAYSIZE(db->common.int_base);
    const TTL_layout_t curr_int_layout = TTL_create_layout(
        tile_current.shape.width, tile_current.shape.width * tile_current.shape.height);
    const TTL_INT_SUB_TENSOR_TYPE result = TTL_create_int_sub_tensor(db->common.int_base[db->common.index],
                                                                     tile_current.shape,
                                                                     curr_int_layout,
